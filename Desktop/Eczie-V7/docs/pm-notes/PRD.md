...
Eczie is a soft, teardrop-shaped little companion with a calming blue gradient body, big glossy eyes, rosy cheeks, and a gentle, reassuring smile that feels warm and safe. In tougher moments, their expression turns slightly worried with subtle rash marks on their body, visually mirroring a flare while still looking caring and empathetic rather than alarming. 
(refer /Users/ayushrao/Desktop/Eczie-V7/hybrid happy eczie.png)
...
|---------|-----------|----------|
| Back Button | Left chevron | Navigate back |
| Mascot Icon | Small Eczie mascot illustration | Displayed inline left of title |
| Title | "EczAI" | Static, centered |
| Menu Button | Three dots | Opens menu (clear history option) |

#### 2.5.2 Mode Toggle

Segmented pill control spanning full width below the header.

| Option | Active State | Inactive State |
|--------|-------------|----------------|
| Skin Coach | Blue filled pill, white bold text | Dark background, muted text |
| Emotional Support | Blue filled pill, white bold text | Dark background, muted text |

Either option can be active. The active mode determines the input placeholder and AI persona.

#### 2.5.3 Empty State (No Conversation)

Displayed when the chat history is empty. Centered vertically in the available space between the mode toggle and the composer.

| Element | Microcopy | Notes |
|---------|-----------|-------|
| Mascot Illustration | Eczie mascot image | Centered, medium size |
| App Name | "Eczie" | Bold, white, below mascot |
| Tagline | "Your eczema companion" | Muted gray, below app name |
| Disclaimer | "EczAI provides informational support and is not a medical diagnosis." | Small muted gray text, centered, below tagline |

#### 2.5.4 Active Chat State

When messages exist, the empty state is replaced by a scrollable message list showing the conversation history. Messages are grouped by date with a centered date separator label (e.g., "Today").

**User message bubble:** Right-aligned, blue background.

**AI message bubble:** Left-aligned, dark card background.

**AI embedded cards (in-message):** Structured cards may appear inline within AI messages for:
- 7-day analysis summaries (metrics: stress level, sleep quality, top trigger)
- Recommendation lists (bulleted action items with icons)

#### 2.5.5 Input Area

| Element | Behavior |
|---------|----------|
| Attachment Button | Paperclip icon on the left; opens image picker for photo attachment |
| Text Input | Mode-dependent placeholder: "Ask about your skin, triggers, or routines…" (Skin Coach) or "Tell me how you're feeling…" (Emotional Support) |
| Send Button | Blue circle with up-arrow icon; enabled only when input is non-empty and not sending |

**DESIGN GAP:**
- Attachment types supported not fully defined (images confirmed; other file types TBD)

#### 2.5.6 Error and Offline States
...
**Microcopy:** "EczAI provides informational support and is not a medical diagnosis."

Shown in the empty state. Hidden once conversation is active.
